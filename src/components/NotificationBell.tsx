...
export default function NotificationBell(){
 const {entries,analysesCount,reports,refreshData}=useAnalysis(); const {subscription}=useAuth(); const navigate=useNavigate();
 ...
 if(latest)list.unshift({id:"analysis-"+latest.id,title:"Analyse terminée",body:"« "+latest.name+" » est disponible dans votre historique.",to:"/analyses",kind:"ok"}); const report=reports[0]; if(report)list.unshift({id:"report-"+report.id,title:"Rapport disponible",body:"Votre rapport est prêt à être consulté.",to:"/reports",kind:"ok"});
 return list.filter(n=>!hidden.includes(n.id)).slice(0,12)},[entries,analysesCount,reports,subscription.plan,subscription.plan,hidden]);
