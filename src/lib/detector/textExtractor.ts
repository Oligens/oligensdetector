...
function validateSignature(ext: string, bytes: Uint8Array): void {
  if (ext === "pdf") {
    const header = new TextDecoder().decode(bytes.slice(0, 5));
    if (header !== "%PDF-") throw new Error("Le fichier PDF n'a pas une signature PDF valide.");
  }
  if (ext === "docx" && !(bytes[0] === 0x50 && bytes[1] === 0x4b)) {
    throw new Error("Le fichier DOCX n'a pas une signature ZIP valide.");
  }
}
...
export async function extractTextFromFile(file: File, onPage?: (page: number, total: number) => void): Promise<string> {
  ...
  if (ext === "pdf") {
    try { return await extractPdf(file, onPage); }
    catch (browserError) {
      try { return await serverFallback(file); }
      catch { throw browserError; }
    }
  }
  if (ext === "docx") {
    try { return await extractDocx(file); }
    catch (browserError) {
      try { return await serverFallback(file); }
      catch { throw browserError; }
    }
