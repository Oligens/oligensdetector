...
import { computeCorrectedOriginality } from "./structuralSignals";
...
export function analyzeCalibrated(text: string, genre = "generic"): HeuristicResult & { features: Features } {
  ...
  const report = [["Expressions standardisées", template * 0.30],["Répétition lexicale", repetition * 0.12],["Rythme régulier", rhythmRegularity * 0.10],["Similarité entre phrases", similarity * 0.10],["Diversité du vocabulaire", -scale(features.mattr, 0.55, 0.88) * 0.18],["Variation des longueurs", -scale(features.burstiness, 0.28, 1.05) * 0.16],["Diversité des débuts", -scale(features.diversiteDebutsPhrase, 1.0, 3.2) * 0.12]].map(([nom, contribution]) => ({ nom: String(nom), z_score: 0, contribution: Number(contribution) })).sort((a,b)=>Math.abs(b.contribution)-Math.abs(a.contribution)).slice(0,7);
