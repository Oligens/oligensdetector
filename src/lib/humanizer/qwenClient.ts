...
export async function humanizeHybrid(text: string, config: Partial<HumanizerConfig>, cb: HybridCallbacks = {}): Promise<HybridOutcome> {
  ...
  cb.onPhase?.("2/3 — Réécriture distante Gemini / Qwen…");
  ...
  try {
    ...
    const report: HumanizerReport = {
      proba_initiale: before.probability,
      proba_finale: after.probability,
      reduction_pourcent: (before.probability - after.probability) * 100,
      ...
      decision: `Détection canonique → réécriture ${flow === "qwen" ? "Qwen" : "Gemini"} → détection canonique terminées.`,
