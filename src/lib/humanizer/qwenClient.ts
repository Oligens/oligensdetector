...
// Historical filename retained for UI compatibility. /api/humanize is now a deterministic local TypeScript engine.
// Scores always come from the canonical /api/detect pipeline.
...
export async function humanizeHybrid(text: string, config: Partial<HumanizerConfig>, cb: HybridCallbacks = {}): Promise<HybridOutcome> {
  ...
  try {
    const response = await fetch("/api/humanize", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, signal: controller.signal, body: JSON.stringify({ text: original, language: config.langue === "fr" || config.langue === "en" ? config.langue : undefined, mode: config.modeAggressif ? "ultra" : "standard", intensity: config.intensite ?? 0.78 }) });
    const data = await response.json().catch(() => ({} as Record<string, unknown>));
    ...
    const report: HumanizerReport = { proba_initiale: before.probability, proba_finale: after.probability, reduction_pourcent: (before.probability - after.probability) * 100, iterations_realisees: 1, historique: [], features_finales: after.factors, viaApi: true, model: typeof data.model === "string" ? data.model : flow === "local" ? "COJ_Local_TS_Humanizer" : flow === "deepseek" ? "deepseek-chat" : "gemini-2.5-flash", decision: `Détection canonique → réécriture ${flow === "local" ? "moteur local TypeScript" : flow === "deepseek" ? "DeepSeek" : "Gemini"} → détection canonique terminées.`, warning: "Le moteur améliore la qualité stylistique et ne garantit pas un score de détection nul.", config: { seuilCible: config.seuilCible ?? 0.12, intensite: config.intensite ?? 0.78, iterationsMax: config.iterationsMax ?? 6, modeAggressif: Boolean(config.modeAggressif), langue: config.langue ?? "mixte" } };
