...
export interface OligensPassage{ id:string; level:"phrase"|"paragraphe"; index:number; text:string; scoreIA:number; confidence:number; verdict:"humain"|"mixte"|"ia"|"insuffisant"; signals:Array<{name:string;value:number;direction:"ia"|"humain"}>; explanation:string }
export interface OligensMLResult{modelVersion:string;scoreIA:number;confidence:number;verdict:"humain"|"mixte"|"ia"|"insuffisant";mixedText:boolean;coverage:number;passages:OligensPassage[];explanation:string;metrics:{precision:number|null;recall:number|null;f1:number|null;falsePositiveRate:number|null;evaluatedSamples:number}}
export interface CopyleaksResult{provider:"copyleaks";scanId:string;modelVersion?:string;aiProbability:number;humanProbability:number;summary:{ai:number;human:number};environment:"production"|"preview"|"development";sandbox:boolean}
export interface GlobalResults{fileName:string;ia:number;plagiat:number;refs:number;human:number;refsTotal:number;refsDouteuses:number;passages:number;summary:string;origins:OriginRow[];confidence?:"Faible"|"Moyenne"|"Élevée";confidenceInterval?:[number,number];decision?:string;engine?:{mode:"direct"|"worker";durationMs:number;words:number};language?:"fr"|"en"|"mixte";signatureNote?:string;topFactors?:Array<{nom:string;z_score:number;contribution:number}>;metrics?:{precision:number|null;transitionDensity:number|null;burstiness:number|null;mattr:number|null;originalite:number|null;charEntropy:number|null};oligensMl?:OligensMLResult;copyleaks?:CopyleaksResult}
...
export function fmtInt(n:number|null|undefined){const value=Number(n);return Number.isFinite(value)?value.toLocaleString("fr-FR"):"0"}
