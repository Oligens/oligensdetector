...
type Res = { status: (n: number) => Res; setHeader: (k: string, v: string) => Res; json: (v: unknown) => void };
...
function send(res: Res, status: number, body: unknown) { res.status(status).setHeader("Content-Type", "application/json").json(body); }
export default async function handler(req: Req, res: Res) {
  ...
  const plan = body.plan; const period = body.billingPeriod; const provider = body.provider ?? "zakapro";
  ...
  if (!/^\+?509[0-9]{8}$/.test(phone)) return send(res, 400, { error: "Numéro haïtien invalide." });
  ...
  const { error: insertError } = await admin.from("payment_transactions").insert({ id: transactionId, user_id: auth.user.id, provider, plan, billing_period: period, amount, currency: "HTG", phone, status: "pending", metadata });
  ...
  try {
    const response = await fetch(ZAKAPRO_API_URL, { method: "POST", headers: { "Content-Type": "application/json", ...(ZAKAPRO_API_KEY ? { Authorization: `Bearer ${ZAKAPRO_API_KEY}` } : {}) }, body: JSON.stringify({ transactionId, provider, plan, billingPeriod: period, amount, currency: "HTG", phone, callbackUrl: `${process.env.APP_URL ?? ""}/api/payments/webhook` }) });
