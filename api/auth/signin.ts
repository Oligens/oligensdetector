...
function getPool() {
  if (pool) return pool;
  pool = new Pool({ connectionString: databaseUrl(), max: 5, idleTimeoutMillis: 10_000, connectionTimeoutMillis: 10_000, ssl: { rejectUnauthorized: true }, application_name: "oligens-detector-auth-signin" });
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const result = await getPool().query<{ id: string; password_hash: string; email_verified: boolean; is_active: boolean }>(
      "SELECT id,password_hash,email_verified,is_active FROM users WHERE email=$1 LIMIT 1", [email],
    );
    const user = result.rows[0];
    if (!user || !user.is_active || !(await bcrypt.compare(password, user.password_hash))) return res.status(401).json({ error: "E-mail ou mot de passe incorrect." });
    ...
    const token = jwt.sign({ sub: user.id }, authSecret(), { expiresIn: "30d", issuer: "oligens-detector" });
