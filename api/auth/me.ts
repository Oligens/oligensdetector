...
function getPool() {
  if (pool) return pool;
  pool = new Pool({
    connectionString: databaseUrl(), max: 5, idleTimeoutMillis: 10_000,
    connectionTimeoutMillis: 10_000, ssl: { rejectUnauthorized: true },
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const userResult = await db.query<AuthUser>(
      "SELECT id,email,email_verified,is_active FROM users WHERE id=$1 LIMIT 1", [payload.sub],
    );
    const user = userResult.rows[0];
    if (!user || !user.is_active) return res.status(401).json({ user: null });

    const subscriptionResult = await db.query<SubscriptionRow>(
      `SELECT
        CASE WHEN plan <> 'free' AND status = 'active' AND expires_at IS NOT NULL AND expires_at <= NOW() THEN 'free'::subscription_plan ELSE plan END AS plan,
        CASE WHEN plan <> 'free' AND status = 'active' AND expires_at IS NOT NULL AND expires_at <= NOW() THEN 'active'::subscription_status ELSE status END AS status,
        CASE WHEN plan <> 'free' AND status = 'active' AND expires_at IS NOT NULL AND expires_at <= NOW() THEN 'monthly'::billing_period ELSE billing_period END AS billing_period,
        CASE WHEN plan <> 'free' AND status = 'active' AND expires_at IS NOT NULL AND expires_at <= NOW() THEN NULL ELSE expires_at END AS expires_at,
        started_at
       FROM subscriptions WHERE user_id=$1 LIMIT 1`, [user.id],
    );
    const subscription = subscriptionResult.rows[0] ?? {
      plan: "free" as const, status: "active" as const, billing_period: "monthly" as const,
      expires_at: null, started_at: new Date().toISOString(),
    };
    ...
    if (subscription.plan === "flash") {
      const usage = await db.query<{ count: string }>(
        `SELECT COUNT(*)::text AS count FROM usage_events
         WHERE user_id=$1 AND event_type='analysis' AND created_at>=DATE_TRUNC('day',NOW())`, [user.id],
      );
    ...
    return res.status(200).json({
      ...
      subscription: {
        plan: subscription.plan, status: subscription.status, period: subscription.billing_period,
