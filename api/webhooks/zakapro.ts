...
const PLAN_BY_ZAKAPRO_ID: Record<string, { plan: "flash"|"pro"|"gold"; period: "monthly"|"yearly" }> = {
  ...
  plan_yv1c14un: { plan: "pro", period: "yearly" },
  plan_c30z8d2g: { plan: "gold", period: "monthly" },
  plan_j6zqhmfv: { plan: "gold", period: "yearly" },
};
...
function db() {
  ...
  pool = new Pool({ connectionString: url, max: 5, idleTimeoutMillis: 10000, connectionTimeoutMillis: 10000, ssl: { rejectUnauthorized: false }, application_name: "oligens-zakapro-webhook" });
  return pool;
}
...
function signatureValid(raw: string, req: VercelRequest) {
  const secret = (process.env.ZAKAPRO_WEBHOOK_SECRET || process.env.ZAKAPRO_APP_SECRET || "").trim();
  if (!secret) return false;
  const provided = String(req.headers["zakapro-signature"] || req.headers["x-zakapro-signature"] || "").trim().replace(/^sha256=/i, "");
  if (!provided) return false;
  const expected = crypto.createHmac("sha256", secret).update(raw, "utf8").digest("hex");
  return provided.length === expected.length && crypto.timingSafeEqual(Buffer.from(provided, "utf8"), Buffer.from(expected, "utf8"));
}
function pick(obj: Record<string, any>, ...keys: string[]) { for (const k of keys) if (obj[k] !== undefined && obj[k] !== null && obj[k] !== "") return obj[k]; return undefined; }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!signatureValid(raw, req)) return res.status(401).json({ error: "Signature ZakaPro invalide." });
    ...
    try {
      ...
      const existing = await client.query<any>(
        `SELECT id,user_id,subscription_id,plan,billing_period,amount,status,phone_number FROM payments
         WHERE provider_transaction_id=$1
            OR ($2 <> '' AND status IN ('pending','processing') AND plan=$3 AND billing_period=$4 AND phone_number=$2)
         ORDER BY created_at DESC LIMIT 1 FOR UPDATE`,
        [reference, phone, mapped.plan, mapped.period]
      );
      const payment = existing.rows[0];
      if (!payment) { await client.query("ROLLBACK"); return res.status(404).json({ error: "Aucun paiement Oligens en attente ne correspond à cette transaction." }); }
      if (successful) {
        const expected = amountNumber(payment.amount);
        if (!Number.isFinite(amount) || Math.abs(amount - expected) > 0.001) {
          await client.query(`UPDATE payments SET status='failed',provider_response=$2::jsonb,webhook_received_at=NOW() WHERE id=$1`, [payment.id, raw]);
          await client.query("COMMIT");
          return res.status(422).json({ error: "Montant reçu différent du montant attendu. Abonnement non activé.", code: "AMOUNT_MISMATCH" });
        }
        await client.query(
          `UPDATE payments SET status='paid',provider_transaction_id=$2,provider_response=$3::jsonb,webhook_received_at=NOW(),paid_at=COALESCE(paid_at,NOW()) WHERE id=$1`,
          [payment.id, reference, raw]
        );
        ...
        return res.status(202).json({ received: true, ignored: true, status });
      }
      await client.query("COMMIT");
      return res.status(200).json({ received: true, activated: successful, reference });
    } catch (e) {
      await client.query("ROLLBACK");
      throw e;
    ...
    return res.status(503).json({ error: "Webhook temporairement indisponible." });
  }
}
