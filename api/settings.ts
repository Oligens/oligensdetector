...
function getPool() {
  ...
  pool = new Pool({
    ...
    application_name: "oligens-detector-settings",
  });
...
const defaults = {
  alert_threshold: 50,
  min_words: 30,
  worker_threshold: 10_000,
  auto_flag: true,
  archive_90_days: true,
  auto_purge: true,
  api_key: "",
  endpoint: "",
};
  ...
  try {
    ...
    if (req.method === "GET") {
      const r = await db.query(
        "SELECT alert_threshold,min_words,worker_threshold,auto_flag,archive_90_days,auto_purge,api_key,endpoint,updated_at FROM user_settings WHERE user_id=$1",
        [id]
      );
      if (!r.rows[0]) {
        const created = await db.query(
          "INSERT INTO user_settings(user_id) VALUES($1) RETURNING alert_threshold,min_words,worker_threshold,auto_flag,archive_90_days,auto_purge,api_key,endpoint,updated_at",
          [id]
        );
    ...
    if (![s.alert_threshold, s.min_words, s.worker_threshold].every(Number.isInteger)) {
      return res.status(400).json({ error: "Paramètres numériques invalides." });
    }

    const r = await db.query(
      `INSERT INTO user_settings(user_id,alert_threshold,min_words,worker_threshold,auto_flag,archive_90_days,auto_purge,api_key,endpoint)
       VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)
       ON CONFLICT(user_id) DO UPDATE SET
         alert_threshold=EXCLUDED.alert_threshold,
         min_words=EXCLUDED.min_words,
         worker_threshold=EXCLUDED.worker_threshold,
         auto_flag=EXCLUDED.auto_flag,
         archive_90_days=EXCLUDED.archive_90_days,
         auto_purge=EXCLUDED.auto_purge,
         api_key=EXCLUDED.api_key,
         endpoint=EXCLUDED.endpoint
       RETURNING alert_threshold,min_words,worker_threshold,auto_flag,archive_90_days,auto_purge,api_key,endpoint,updated_at`,
      [id, s.alert_threshold, s.min_words, s.worker_threshold, s.auto_flag, s.archive_90_days, s.auto_purge, s.api_key, s.endpoint]
    );
    return res.status(200).json({ settings: publicSettings(r.rows[0]) });
  } catch (error) {
