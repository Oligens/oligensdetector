...
import { humanizeWithQwen, qwenConfiguration } from "../src/lib/ai/qwenService";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const providers: Array<"gemini" | "qwen"> = [];
    ...
    if (geminiConfiguration().configured) {
      try {
        ...
        return res.status(200).json({ success: true, ...result, providerChain: providers });
      } catch (error) { lastError = error instanceof Error ? error.message : "Gemini indisponible."; }
    }

    if (qwenConfiguration().configured) {
      try {
        const result = await humanizeWithQwen(text, { language, mode });
        if (result.text.trim() === text) throw new Error("Qwen a renvoyé le texte source inchangé.");
        providers.push("qwen");
        return res.status(200).json({ success: true, ...result, providerChain: providers, fallbackFrom: lastError });
      } catch (error) { lastError = error instanceof Error ? error.message : "Qwen indisponible."; }
    }

    return res.status(503).json({ error: lastError, code: "NO_HUMANIZER_PROVIDER", gemini: geminiConfiguration(), qwen: qwenConfiguration() });
  } catch (error) {
